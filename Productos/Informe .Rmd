...
Se observa que, existen dos categorías que se pueden fusionar (*Graphic Novels: Manga* y *Graphic Novels: Anime & Manga* ), provocando que esta unión lidere el Top-Seller, tal como se aprecia en el siguiente gráfico:
...
Las dos figuras anteriores muestran que existe una tendencia muy marcada, en comparación con los autores, en las preferencias de categorías literarias, donde lideran *Graphic Novels: Anime & Manga* y *Contemporary Fiction* con más de 1000 libros cada uno, dentro del Top-Seller. Luego, si se comparan las frecuencias de estas con los autores más populares, se puede cuestionar la existencia de una relación entre ellos, es decir, se pone en duda que los 10 autores del Top-Seller escriben libros clasificados en las categorías vistas en el gráfico.
...
Se observa que, al intersectar las categorías con los autores, desaparecen *Graphic Novels: Anime & Manga* y *Sci Fi Books for Kids*. Esto se debe a que ninguno de los 10 principales autores es dueño de algún libro clasificado en esas categorías. Sin embargo, se observa el caso contrario con algunos autores, es decir, poseen muchos libros en una categoría. Por ejemplo, J.K. Rowling en *Fantasy Books for Kids*. 

En consecuencia, se podría insinuar que existe una especialización que provoca que la escritora posea una buena calificación promedio en el rating_avg. Sin embargo, esto se descarta al considerar las puntuaciones de libros individuales con puntuación perfecta (5), independientemente del autor, como se muestra a continuación:

```{r echo=FALSE}
puntuacion_perfecta <- por_autor %>% 
  filter(rating_avg == 5) %>% 
  filter(author_name %in% autores_crudo$author_name[1:3]) %>% 
  separate_rows(categories, sep = ", ") %>% 
  mutate(categories = as.numeric(str_remove_all(categories, "\\[|\\]"))) %>% 
  left_join(Categorias, by = c("categories" = "category_id")) %>% 
...
Esta tabla muestra todos los libros que presentan categorías y escritores pertenecientes al top 10, respectivamente. Entonces, considerando estos datos en conjunto con el último gráfico de barras, se aprecia que un autor puede tener una puntuación perfecta en rating_avg, aunque no se 'especialice' en una categoría en específica, como es el caso de Axel Scheffler, quien posee libros en 5 categorías distintas. 
...
2) bestsellers-rank: ID-Ranking de los artículos más vendidos (int)
...
17) imprint: Lamentablemente no hay información registrada
...
29) weight: Peso (en kg)

### Authors

Contiene 2 columnas:

1) author_id: ID-Autor (int)

2) author_name: Nombre del autor correspondiente al ID (str)

### Categories

Contiene 2 columnas:

1) category_id: ID-Categoría (int)

2) category_name: Nombre de la categoría correspondiente al ID (str)

### Formats

Contiene 2 columnas:

1) format_id: ID-Formato (int)

2) format_name: Nombre del formato correspondiente al ID (str)

### Places

Contiene 2 columas:

1) place_id: Sin información

2) place_name: Sin información. 

### Datos Finales

Tal como se mencionó en Datos, datos_finales.csv es el resultado del pre-procesamiento de dataset.csv, que se puede ver en detalle en el siguiente [enlace](https://es.wikipedia.org)


1) authors :  ID-Autor(es) (lista de str)

2) bestsellers_rank : ID-Ranking de los artículos más vendidos (int)

3) categories : ID-Categorías. (lista de str)

4) format : ID-Formato. (int)

5) id : Identificador único (int)

6) lang : Idioma del artículo

7) rating_avg : Calificación promedio 0-5

8) title : Título del libro (str)
